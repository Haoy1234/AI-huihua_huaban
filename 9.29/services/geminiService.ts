...
): Promise<{ newImages: { base64: string; mimeType: string; }[]; textResponse: string | null; }> {
  try {
    ...
    if (response.generatedImages && response.generatedImages.length > 0) {
      return {
        newImages: response.generatedImages.map(image => ({
...
export async function generateVideo(
  ...
  onProgress(`Downloading ${downloadLinks.length} generated video(s)...`);

  const videoPromises = downloadLinks.map(async (link) => {
    const response = await fetch(`${link}&key=${API_KEY}`);
    if (!response.ok) {
