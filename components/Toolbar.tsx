...
import type { AIServiceProvider } from '../services/aiServiceManager.ts';
import { getServiceConfig } from '../services/aiServiceManager.ts';
...
interface ToolbarProps {
    ...
    setDrawingOptions: (options: { strokeColor: string; strokeWidth: number; opacity?: number }) => void;
    ...
    canRedo: boolean;
    // AI服务选择器相关属性
    selectedAIProvider?: AIServiceProvider;
    onAIProviderChange?: (provider: AIServiceProvider) => void;
    availableAIProviders?: AIServiceProvider[];
    hideEntries?: { boards?: boolean; layers?: boolean; settings?: boolean };
}
...
export const Toolbar: React.FC<ToolbarProps> = ({
    ...
    return (
        ...
        >
            ...
            {/* AI服务选择器 */}
            {availableAIProviders && availableAIProviders.length > 1 && (
                <div className="flex flex-col items-center gap-1">
                    <div className="text-xs text-white/70 font-medium">AI服务</div>
                    <select
                        value={selectedAIProvider || 'openrouter'}
                        onChange={(e) => onAIProviderChange?.(e.target.value as AIServiceProvider)}
                        className="w-16 px-1 py-1 bg-white/10 text-white rounded border border-white/20 text-xs focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
                        title="选择AI服务提供商"
                    >
                        {availableAIProviders.map(provider => {
                            const config = getServiceConfig(provider);
                            return (
                                <option key={provider} value={provider} className="text-black text-xs">
                                    {config.displayName}
                                </option>
                            );
                        })}
            ...
            <div className="flex flex-col items-center gap-2 flex-grow">
                ...
                <input 
                    ...
                    max="1" 
                    step="0.05" 
                    value={drawingOptions.opacity ?? 1} 
                    aria-label={t('toolbar.opacity')} 
                    title={t('toolbar.opacity')} 
                    onChange={(e) => setDrawingOptions({ ...drawingOptions, opacity: parseFloat(e.target.value) })} 
                    className="w-10 cursor-pointer" 
                />
                <span className="text-sm text-white w-6 text-center">{Math.round((drawingOptions.opacity ?? 1) * 100)}%</span>
                <div className="w-10 h-px bg-white/30"></div>
